...
nmecr contains four data modeling algorithms. These create the data models, report on the models' goodness-of-fit metrics, provide details on the model residuals' skewness, and predict using the developed model when given a prediction dataset. 
...
The California Public Utility Commission requires savings uncertainty for 10% savings at the 90% confidence level to be reported to qualify a model as 'good' and fit for use in energy efficieny programs. You can obtain this information for your model using the `calculate_savings_uncertainty()` function:
...
The goodness of fit metrics (CV(RMSE) < 25%, R^2 > 70%, NDBE < 0.005%), the time series plot, the residual analysis, and the savings uncertainty calculations show the developed model is good and fit for prediction.
...
actual_savings_uncertainty <- calculate_savings_uncertainty(modeled_data_obj = TOWT_predictions, 
                              savings_percent = avoided_energy_use$pct_savings)
