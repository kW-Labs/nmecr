...
For an overview of nmcer, please refer [nmecr_overview.pdf](https://github.com/kW-Labs/nmecr/tree/master/vignettes/nmecr.pdf)
...
As noted in the [nmecr_predictability.pdf](https://github.com/kW-Labs/nmecr/tree/master/vignettes/nmecr_predictability.pdf) vignette, the Time-of-Week and Temperature model is best suited for this facility. Here we create a TOWT model for the baseline as well as the performance period data: 
...
`calculate_summary_statistics()` can be used to calculate the model fit statistics for both models:
...
Both models have CV(RMSE) values lower than 25%
...
3. Savings Uncertainty for 10% savings (at 90% confidence level) (shoule be < 50%):

Baseline Period: 16.7%
Performance Period: 19.8%

In addition to evaluating the model metrics, it is essential to ensure that the modeled profile follows the actual energy use closely (see Figure 8 above), and that the four assumptions of linear regression are met by the model. For a step-by-step process of evaluating the model against the four assumptions of regression, read [nmecr_predictability.pdf](https://github.com/kW-Labs/nmecr/tree/master/vignettes/nmecr_predictability.pdf)

## Normalized Savings

To calculate Normalized Savings, the energy use under normal conditions needs to be predicted. Inspecting the TMY3 data, we see that it has hourly data intervals:
...
```{r baseline normalized use, results='markup'}

TOWT_baseline_normalized <- calculate_model_predictions(training_data = TOWT_baseline_df, prediction_data = TMY3_daily, modeled_object = TOWT_baseline_model)
TOWT_performance_normalized <- calculate_model_predictions(training_data = TOWT_performance_df, prediction_data = TMY3_daily, modeled_object = TOWT_performance_model)
