...
create_dataframe <-
  function(eload_data = NULL,
    ...
    if (!missing("operating_mode_data")) {
      warning(
        "'operating_mode_data' has been deprecated and will be discontinued in future releases. Please use 'additional_independent_variables' instead."
      )
      additional_independent_variables <- operating_mode_data
    }

    if (!is.null(additional_independent_variables)) {
      additional_variables_names <-
        colnames(additional_independent_variables)
      additional_variables_count <-
        length(additional_variables_names) - 1
      if (additional_variables_count > length(additional_variable_aggregation)) {
        stop(
          "Please provide an aggregation function for each of the additional variables input. Use argument 'additional_variable_aggregation' to specify the aggregation functions."
        )
      }
    ...
    if (!lubridate::is.POSIXct(eload_data$time)) {
      stop(
        "Timestamps in 'eload_data' are not Date-Time objects. Please use the 'lubridate' package to parse in the timestamps appropriately."
      )
    }

    if (!lubridate::is.POSIXct(temp_data$time)) {
      stop(
        "Timestamps in 'temp_data' are not Date-Time objects. Please use the 'lubridate' package to parse in the timestamps appropriately."
      )
    }
    ...
    if (!is.null(additional_independent_variables)) {
      if (!is.POSIXct(additional_independent_variables$time)) {
        stop(
          "Timestamps in 'additional_independent_variables' are not Date-Time objects. Please use the 'lubridate' package to parse in the timestamps appropriately."
        )
      }
    }
    ...
    if (!is.null(start_date)) {
      if (!is.character(start_date)) {
        stop(
          "Enter the start date as a character string, preferably in the 'mm/dd/yyyy hh:mm' format"
        )
      }
    }
    ...
    if (!is.null(end_date)) {
      if (!is.character(end_date)) {
        stop(
          "Enter the end date as a character string, preferably in the 'mm/dd/yyyy hh:mm' format"
        )
      }
    }
    ...
    if (!is.null(additional_independent_variables)) {
      additional_independent_variables <-
        as.data.frame(additional_independent_variables)
    }

    eload_data_xts <-
      xts::xts(x = eload_data[,-1], order.by = eload_data[, 1])
    temp_data_xts <-
      xts::xts(x = temp_data[,-1], order.by = temp_data[, 1])
    if (!is.null(additional_independent_variables)) {
      additional_independent_variables_xts <-
        xts::xts(x = additional_independent_variables[,-1], order.by = additional_independent_variables[, 1])
    }
    ...
    if (!is.null(additional_independent_variables)) {
      additional_independent_variables_names <-
        colnames(additional_independent_variables)[-1]
      colnames(additional_independent_variables_xts) <-
        additional_independent_variables_names
    }

    # Check if input is less than 15-min data. Not allowing less than 15-min data as of now. May change in the future.

    if (xts::periodicity(eload_data_xts)['frequency'] < 15 &
        xts::periodicity(eload_data_xts)['scale'] == 'minute') {
      stop("Cannot input data with frequency of less than 15-minutes")
    }

    if (xts::periodicity(temp_data_xts)['frequency'] < 15 &
        xts::periodicity(temp_data_xts)['scale'] == 'minute') {
      stop("Cannot input data with frequency of less than 15-minutes")
    }

    if (!is.null(additional_independent_variables)) {
      if (xts::periodicity(additional_independent_variables_xts)['frequency'] < 15 &
          xts::periodicity(additional_independent_variables_xts)['scale'] == 'minute') {
        stop("Cannot input data with frequency of less than 15-minutes")
      }
    }
    ...
    if (!is.null(additional_independent_variables)) {
      scale_additional_independent_variables <-
        xts::periodicity(additional_independent_variables_xts)['scale']
    }
    ...
    if (!is.null(additional_independent_variables)) {
      additional_independent_variables_interval <-
        determine_data_interval_sec(scale_additional_independent_variables)
    }
    ...
    if (missing(convert_to_data_interval)) {
      ...
      if (max_data_interval > 60 * 15) {
        warning(
          "Uploaded datasets have data intervals of greater than 15 minutes. Aggregating to the highest data interval."
        )
        nterval <- max_data_interval
      } else {
        nterval <- 60 * 15
      ...
      if (max_data_interval > 60 * 60) {
        warning(
          "Uploaded datasets have data intervals of greater than 1 hour. Aggregating to the highest data interval."
        )
        nterval <- max_data_interval
      } else {
        nterval <- 60 * 60
      ...
      if (max_data_interval > 60 * 60 * 24) {
        warning(
          "Uploaded datasets have data intervals of greater than 1 day. Aggregating to the highest data interval."
        )
        nterval <- max_data_interval
      } else {
        nterval <- 60 * 60 * 24
      ...
      stop(
        paste0(
          "Check spellings: convert_to_data_interval can be specified as one of the following:
    ...
    if (!is.null(additional_independent_variables)) {
      ...
      for (i in 1:length(additional_independent_variables_names)) {
        dfs[i] <-
          data.frame(additional_independent_variables[additional_independent_variables_names[i]])
      }

      names(dfs) <- additional_independent_variables_names

      apply_aggregation <-
        function(df, aggregation_function, nterval) {
          # Function for aggregating individual columns of the additional_independent_variables input
          ...
          df <- data.frame(df)
          df <-
            data.frame("time" = additional_independent_variables$time) %>%
            bind_cols(df)
          ...
          if (nterval == 60 * 60 |
            ...
            aggregated_df_xts <-
              xts::period.apply(
                df_xts,
                INDEX = xts::endpoints(df_xts, xts_index),
    ...
    # Begin aggregation
    if (nterval == 60 * 15) {
      # if the convert_to_data_interval input is '15-min' or max_data_interval (max interval based on input datasets) == 15 min

      if (!is.null(additional_independent_variables)) {
        data_xts <-
          xts::merge.xts(eload_data_xts,
                         temp_data_xts,
                         additional_independent_variables_xts)
      } else {
    ...
    } else if (nterval == 60 * 60) {
      # if the convert_to_data_interval input is 'Hourly' or max_data_interval == 60*60

      sum_eload_data_xts <-
        xts::period.apply(
          eload_data_xts$eload,
          INDEX = xts::endpoints(eload_data_xts, "hours"),
          FUN = sum,
          na.rm = T
        ) %>%
      ...
      mean_temp_data_xts <-
        xts::period.apply(
          temp_data_xts$temp,
          INDEX = xts::endpoints(temp_data_xts, "hours"),
          FUN = mean,
          na.rm = T
        ) %>%
        xts::align.time(n = 60 * 60)
      ...
      if (!is.null(additional_independent_variables)) {
        aggregated_dfs_xts <-
          purrr::map2(
            ...
            .f = apply_aggregation,
            nterval = 60 * 60
          ) # dfs calculated above

        aggregated_df_xts <-  do.call('cbind', aggregated_dfs_xts)
        names(aggregated_df_xts) <-
          additional_independent_variables_names

        data_xts <-
          xts::merge.xts(sum_eload_data_xts,
                         mean_temp_data_xts,
                         aggregated_df_xts)
      ...
      # if the convert_to_data_interval input is 'Daily' or max_data_interval == 60*60*24. HDD and CDDs being calculated as well.

      sum_eload_data_xts <-
        xts::period.apply(
          eload_data_xts$eload,
          INDEX = xts::endpoints(eload_data_xts, "days"),
          FUN = sum,
          na.rm = T
        ) %>%
      ...
      mean_temp_data_xts <-
        xts::period.apply(
          temp_data_xts$temp,
          INDEX = xts::endpoints(temp_data_xts, "days"),
          FUN = mean,
          na.rm = T
        ) %>%
        xts::align.time(n = 60 * 60 * 24)
      ...
      # if the convert_to_data_interval input is 'Monthly'

      determine_alignment <- function(nterval) {
        if (nterval == 60 * 15 | nterval == 60 * 60) {
          align_to_minutes <-
            60 # aligning to 60 minutes because when aggregating to Monthly level, we want to get the rounded hour
        } else if (nterval == 60 * 60 * 24) {
      ...
      if (!is.null(additional_independent_variables)) {
        additional_variable_alignment <-
          determine_alignment(additional_independent_variables_interval)
      }

      if (!is.null(eload_alignment)) {
        # aggregate up if less than monthly
        sum_eload_data_xts <-
          xts::period.apply(
            eload_data_xts$eload,
            INDEX = xts::endpoints(eload_data_xts, "months"),
            FUN = sum,
            na.rm = T
          ) %>%
        ...
        sum_eload_data_xts <-
          eload_data_xts # if input as monthly - assumption is that the its aligned to the end of the period
      }

      if (!is.null(temp_alignment)) {
        # aggregate up if less than monthly
        ...
        daily_data <-
          dplyr::bind_cols(daily_temp, "HDD" = HDD, "CDD" = CDD)

        monthly_temp <-
          data.frame(matrix(ncol = 5, nrow = length(sum_eload_data_xts)))
        names(monthly_temp) <- c("time", "temp", "HDD", "CDD", "days")
        monthly_temp$time <-
          index(sum_eload_data_xts)# aligning to eload - representing the end of the period

        for (i in 2:length(monthly_temp$time)) {
          monthly_temp$temp[i] <- daily_data %>%
            dplyr::filter(time >= monthly_temp$time[i - 1] &
                            time < monthly_temp$time[i]) %>%
          ...
          monthly_temp$days[i] <-
            difftime(monthly_temp$time[i], monthly_temp$time[i - 1], units = "days") %>%
            as.numeric()
        }

        # Fill in the first row
        monthly_temp$temp[1] <- daily_data %>%
          dplyr::filter(time >= monthly_temp$time[1] - 2592000 &
                          time < monthly_temp$time[1]) %>%
          dplyr::summarize("temp" = round(mean(temp), 2)) %>%
          as.numeric()
        ...
        monthly_temp$days[1] <-
          difftime(monthly_temp$time[1], monthly_temp$time[1] - 2592000, units = "days") %>%
          as.numeric()

        mean_temp_data_xts <-
          xts::xts(x = monthly_temp[,-1], order.by = monthly_temp[, 1])

      } else {
        mean_temp_data_xts <-
          temp_data_xts # if input as monthly - assumption is that the its aligned to the end of the period
        mean_temp_data_xts$days <-
          lubridate::days_in_month(month(index(mean_temp_data_xts) - 30))
        mean_temp_data_xts$HDD <-
          (temp_balancepoint - mean_temp_data_xts$temp) * mean_temp_data_xts$days
        mean_temp_data_xts$HDD[mean_temp_data_xts$HDD < 0] <- 0
        mean_temp_data_xts$CDD <-
          (mean_temp_data_xts$temp - temp_balancepoint) * mean_temp_data_xts$days
      ...
      if (!is.null(additional_independent_variables)) {
        if (!is.null(additional_variable_alignment)) {
          # aggregate up if less than monthly
          aggregated_dfs_xts <-
            purrr::map2(
              .x = dfs,
    ...
    if (!is.null(start_date)) {
      ...
      if (is.na(check_start_date)) {
        start_date <-
          ...
          paste(hour(start_date), minute(start_date), sep = ":"),
          sep = " ")
      }
      data_xts <-
        data_xts[zoo::index(data_xts) >= lubridate::mdy_hm(start_date)]
    }
    ...
    if (!is.null(end_date)) {
      ...
      if (is.na(check_end_date)) {
        end_date <-
          paste(paste(month(end_date), day(end_date), year(end_date), sep = "/"),
                paste(hour(end_date), minute(end_date), sep = ":"),
                sep = " ")
      }

      data_xts <-
        data_xts[zoo::index(data_xts) <= lubridate::mdy_hm(end_date)]
    }

    df <-
      timetk::tk_tbl(data = data_xts,
                     preserve_index = TRUE,
                     rename_index = "time") # return a df
    ...
    if (nterval == 2592000) {
      ...
      normalized_df <-
        data.frame(matrix(nrow = nrow(df), ncol = length(df_columns)))
      names(normalized_df) <- paste0(df_columns, "_perday")

      for (i in 1:length(df_columns)) {
        normalized_df[i] <-
          normalize_by_days(df_col = df[df_columns[i]], days_col = df$days)
      }
